...
async function handleOrderUpdated(order) {
  ...
  // Payment status synchronization + stage mapping (category 2)
  const fs = (order.financial_status || '').toLowerCase();
  if (fs === 'paid') {
    fields.UF_CRM_PAYMENT_STATUS = 'PAID';
    fields.STAGE_ID = BITRIX_CONFIG.STAGES.PAID || 'C2:WON';
  } else if (fs === 'partially_paid') {
    fields.UF_CRM_PAYMENT_STATUS = 'PARTIALLY_PAID';
    fields.STAGE_ID = BITRIX_CONFIG.STAGES.PENDING || 'C2:PREPARATION';
  } else if (fs === 'pending') {
    fields.UF_CRM_PAYMENT_STATUS = 'NOT_PAID';
    fields.STAGE_ID = BITRIX_CONFIG.STAGES.PENDING || 'C2:PREPARATION';
  } else if (fs === 'refunded') {
    fields.UF_CRM_PAYMENT_STATUS = 'REFUNDED';
    fields.STAGE_ID = BITRIX_CONFIG.STAGES.REFUNDED || 'C2:LOSE';
  } else if (fs === 'partially_refunded') {
    fields.UF_CRM_PAYMENT_STATUS = 'PARTIALLY_REFUNDED';
    fields.STAGE_ID = BITRIX_CONFIG.STAGES.REFUNDED || 'C2:LOSE';
  } else if (fs === 'voided') {
    fields.UF_CRM_PAYMENT_STATUS = 'VOIDED';
    fields.STAGE_ID = BITRIX_CONFIG.STAGES.REFUNDED || 'C2:LOSE';
  } else if (fs === 'cancelled') {
    fields.UF_CRM_PAYMENT_STATUS = 'NOT_PAID';
    fields.STAGE_ID = BITRIX_CONFIG.STAGES.CANCELLED || 'C2:LOSE';
  }
