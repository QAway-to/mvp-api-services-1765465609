...
// Optional whitelist for preorder routing
...
// Determine if any line item has positive inventory
function hasStock(lineItems = []) {
  return lineItems.some((li) => {
    // Prefer inventory_quantity if present; otherwise, fallback to fulfillable_quantity
    const qty = typeof li.inventory_quantity === 'number'
...
// Determine if a line item qualifies for preorder
function isPreorderItem(lineItem) {
  ...
  const tags = Array.isArray(tagsRaw)
    ? tagsRaw
    : String(tagsRaw).split(',').map((t) => t.trim());
  const hasPreorderTag = tags.some((t) => t.toLowerCase() === 'preorder');

  // Metafield: custom.preorder == true
  const mfPreorder = product?.metafields?.custom?.preorder === true;

  // Whitelist by SKU or handle
  const sku = lineItem.sku || '';
  const handle = product.handle || '';
  const inWhitelist = !!(
    preOrderWhitelist &&
    (preOrderWhitelist[sku] || preOrderWhitelist[handle])
  );

  return hasPreorderTag || mfPreorder || inWhitelist;
}

function shouldRouteToPreorder(lineItems = []) {
  // Only when no stock and at least one preorder-qualified item
  const anyPreorder = lineItems.some(isPreorderItem);
  return anyPreorder;
}
...
export function mapShopifyOrderToBitrixDeal(order) {
  ...
  // Determine category by stock / preorder rules (Shopify)
  const lineItems = order.line_items || [];
  let categoryId = 2; // Default: Stock (site)
  if (hasStock(lineItems)) {
    categoryId = 2;
  } else if (shouldRouteToPreorder(lineItems)) {
    categoryId = 8; // Pre-order (site)
  } else {
    categoryId = 2; // Fallback to stock site
  }
